...
Player:GetMouse().KeyDown:Connect(function(Key)
    ...
    if getgenv().KeyTrigger.Macro ~= "" then  -- Check if a key is defined for Macro
...
if getgenv().AntiLock.Settings.Enable then
    ...
    getgenv().key = tostring(getgenv().KeyTrigger.AntiLock):lower()  -- Update key reference
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed then  -- Ignore input if it's being processed by a GUI
        if input.KeyCode == getgenv().KeyTrigger.Spin then  -- Update key reference
            performSpin()
        end
    end
end)
...
local function onKeyPress(input, gameProcessed)
    ...
    if input.KeyCode == getgenv().KeyTrigger.Rejoin and not gameProcessed then  -- Update key reference
...
UserInputService.InputBegan:Connect(function(input)
    ...
    if getgenv().PanicSystem.Settings.IsActive then
        -- Check if the pressed key matches the defined keybind from KeyTrigger
